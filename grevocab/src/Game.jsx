...
import './Game.css'
    ...
    const cardAnswers = {
        ...
        ['acclaim', 'ambiguity', 'adversity', 'antagonism', 'apathy'],
        ...
        ['compelling', 'weak', 'ineffective', 'unconvincing', 'persuasive'],
        'Her ________ personality made her the life of every party.':
        ['gregarious', 'reclusive', 'introverted', 'reserved', 'solitary'],
        ...
        ['engaging', 'boring', 'uninteresting', 'dull', 'tedious'],
        ...
        ['airtight', 'flawed', 'weak', 'inadequate', 'questionable'],
        'His ________ demeanor made it easy for people to approach him.':
        ['approachable', 'aloof', 'distant', 'unfriendly', 'reserved'],
        ...
        ['lyrical', 'technical', 'mechanical', 'plain', 'simple'],
    }
...
function handleAnswerClick(prompt, answer, setNewPrompt, selectedAnswer, setSelectedAnswer) {
    ...
    // setNewPrompt(Math.floor(Math.random() * keysArray.length));
}
...
function Card({prompt, setNewPrompt}) {
    ...
    const [shuffledList, setShuffledList] = useState(shuffleList(cardAnswers[prompt]));
    console.log(selectedAnswer);
    console.log(cardAnswers[prompt][0] === selectedAnswer ? 'correct' : 'incorrect');
    return (
        <div className="card">
            ...
            {shuffledList.map((answer, index) => 
                (<li key={answer} className={(cardAnswers[prompt][0] === selectedAnswer && selectedAnswer === answer) ? 'correct' : 'incorrect'} onClick={() => handleAnswerClick(prompt, answer, setNewPrompt, selectedAnswer, setSelectedAnswer)}> {answer} </li>)
            )}
            </ul>
        </div>
    );
